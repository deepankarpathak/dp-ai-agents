...
function formatInline(t) {
  return (t || "")
    ...
    .replace(/`(.*?)`/g, '<code style="background:#1e1e38;padding:1px 6px;border-radius:3px;font-family:JetBrains Mono,monospace;color:#a78bfa;font-size:0.82em">$1</code>');
}
...
function MarkdownRenderer({ content }) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("|") && i + 1 < lines.length && lines[i+1].match(/^\|[\s\-:|]+\|/)) {
      ...
      result.push(`<div style="overflow-x:auto;margin:14px 0;border-radius:8px;overflow:hidden;border:1px solid #1e1e38">
        <table style="width:100%;border-collapse:collapse;font-size:12.5px;font-family:JetBrains Mono,monospace">
          <thead><tr>${headers.map(h=>`<th style="background:#131326;color:#e8b84b;padding:10px 14px;text-align:left;font-weight:600;white-space:nowrap;border-bottom:1px solid #2a2a4a;letter-spacing:0.04em;font-size:11px;text-transform:uppercase">${formatInline(h)}</th>`).join("")}</tr></thead>
          <tbody>${rows.map((r,ri)=>`<tr style="background:${ri%2?"#0c0c1e":"#0f0f22"};transition:background 0.15s" onmouseover="this.style.background='#141430'" onmouseout="this.style.background='${ri%2?"#0c0c1e":"#0f0f22"}'">${r.map((c,ci)=>`<td style="padding:9px 14px;border-bottom:1px solid #131326;color:#cbd5e1;vertical-align:top">${isResultCol(headers[ci]) ? colorResult(c) : formatInline(c)}</td>`).join("")}</tr>`).join("")}</tbody>
        </table></div>`);
      continue;
    }
    ...
    else if (line.match(/^---+$/)) result.push(`<hr style="border:none;border-top:1px solid #1e1e38;margin:18px 0">`);
...
// ── Shared UI atoms ───────────────────────────────────────────────────────────
const C = {
  bg: "#080814", surface: "#0e0e1e", elevated: "#131326", border: "#1e1e38",
        ...
        <div key={i} style={{ display:"flex", alignItems:"center", flex: i<steps.length-1?1:"none" }}>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
            <div style={{
              ...
              background: step>i?C.gold:step===i?`${C.gold}22`:"#1a1a2e",
...
export default function TestSentinel() {
  ...
  const callClaude = async (systemPrompt, userMessage) => {
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:4000, system:systemPrompt, messages:[{role:"user",content:userMessage}], ...(tools?{tools}:{}) })
    });
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", color:C.text }}>
      ...
      <footer style={{ borderTop:`1px solid ${C.border}`, padding:"14px 28px", textAlign:"center", fontSize:10, color:"#2a2a4a", fontFamily:C.mono }}>
