...
async function loadHistory(){ try{ const r=await window.storage.get(HISTORY_KEY); return r?JSON.parse(r.value):[]; }catch{return[];} }
async function saveHistory(h){ try{ await window.storage.set(HISTORY_KEY,JSON.stringify(h)); }catch(e){console.warn(e);} }
  ...
  return (
    <div style={{ position:"fixed", top:0, right:0, bottom:0, width:420, background:"#060D1A", borderLeft:"1px solid #1E293B", zIndex:100, display:"flex", flexDirection:"column", animation:"slideIn .25s ease" }}>
      <div style={{ padding:"16px 20px", borderBottom:"1px solid #1E293B", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div><div style={{ fontSize:14, fontWeight:700, color:"#F1F5F9" }}>📋 PRD History</div><div style={{ fontSize:11, color:"#374151" }}>{history.length} saved</div></div>
...
export default function PRDAgent() {
  ...
  const generateInBatches = async (req) => {
    ...
    for (let i=0; i<BATCHES.length; i++) {
      ...
      let parsed; try{ parsed=repairJSON(raw); }catch{ parsed={}; batch.forEach(k=>{parsed[k]="(Generation failed — please retry)";}); }
  ...
  const persistToHistory = async (prdData, req) => {
    const entry = { id:Date.now().toString(), date:new Date().toLocaleString("en-IN",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"}), model:model.label, requirement:req.slice(0,200), prd:prdData };
  ...
  const handleRefine = async () => {
    ...
    if (!filled.length) { setError("Please answer at least one question."); return; }
